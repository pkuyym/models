...
def reader_creator(tar_file, file_name, dict_size, src_init):
    def reader():
            ...
            for name in names:
                for line in f.extractfile(name):
                    ...
                    if src_init == False:
                        ...
                        src_init_weight = [0 for i in xrange(len(src_ids))]
                        yield src_ids, src_init_weight, trg_ids, trg_ids_next

    return reader


def train(dict_size, src_init=False):
    """
...
def test(dict_size, src_init=False):
    """
...
def gen(dict_size):
    return reader_creator(
        download(URL_TRAIN, 'wmt14', MD5_TRAIN), 'gen/gen', dict_size, src_init)
